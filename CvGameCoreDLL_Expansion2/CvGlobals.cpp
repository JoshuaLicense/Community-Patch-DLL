...
// -- post defines --
	...
	GD_INT_INIT(SUPPLY_TRUCKS_MOVEMENT, 6),
	GD_INT_INIT(SUPPLY_TRUCKS_PLOT_LIMIT, 20),

	GD_INT_INIT(CITY_MAX_PLOT_CONTROL_RANGE, 6),
	GD_INT_INIT(UNIT_MAX_PLOT_CONTROL_RANGE, 3),

	GD_INT_INIT(MAX_HEALING_PERCENTAGE, 10),
	GD_INT_INIT(HEALING_HIGH_DANGER, 15000),
	GD_INT_INIT(HEALING_MEDIUM_DANGER, 5000),
	GD_INT_INIT(HEALING_LOW_DANGER, 1),

	GD_INT_INIT(HEAVY_FUEL_RATIONING, 500),
	GD_INT_INIT(MEDIUM_FUEL_RATIONING, 1000),
	GD_INT_INIT(LIGHT_FUEL_RATIONING, 2500),

	GD_INT_INIT(HEAVY_RATIONING_MOVES, 30),
	GD_INT_INIT(MEDIUM_RATIONING_MOVES, 60),
	GD_INT_INIT(LIGHT_RATIONING_MOVES, 80),

	GD_INT_INIT(HEAVY_RATIONING_CONSUMPTION, 50),
	GD_INT_INIT(MEDIUM_RATIONING_CONSUMPTION, 65),
	GD_INT_INIT(LIGHT_RATIONING_CONSUMPTION, 85),

	GD_INT_INIT(OCCUPIED_PRODUCTION_MODIFIER, -25),
	GD_INT_INIT(OCCUPIED_CITY_CONNECTION_MODIFIER, -25),
#endif
...
void CvGlobals::cacheGlobals()
{
	...
	GD_INT_CACHE(SUPPLY_TRUCKS_PLOT_LIMIT);

	GD_INT_CACHE(CITY_MAX_PLOT_CONTROL_RANGE);
	GD_INT_CACHE(UNIT_MAX_PLOT_CONTROL_RANGE);

	GD_INT_CACHE(MAX_HEALING_PERCENTAGE);
	GD_INT_CACHE(HEALING_HIGH_DANGER);
	GD_INT_CACHE(HEALING_MEDIUM_DANGER);
	GD_INT_CACHE(HEALING_LOW_DANGER);

	GD_INT_CACHE(HEAVY_FUEL_RATIONING);
	GD_INT_CACHE(MEDIUM_FUEL_RATIONING);
	GD_INT_CACHE(LIGHT_FUEL_RATIONING);

	GD_INT_CACHE(HEAVY_RATIONING_MOVES);
	GD_INT_CACHE(MEDIUM_RATIONING_MOVES);
	GD_INT_CACHE(LIGHT_RATIONING_MOVES);

	GD_INT_CACHE(HEAVY_RATIONING_CONSUMPTION);
	GD_INT_CACHE(MEDIUM_RATIONING_CONSUMPTION);
	GD_INT_CACHE(LIGHT_RATIONING_CONSUMPTION);

	GD_INT_CACHE(OCCUPIED_PRODUCTION_MODIFIER);
	GD_INT_CACHE(OCCUPIED_CITY_CONNECTION_MODIFIER);
#endif
}
